...
integer :: k_hf_start = 2
integer :: k_hf_end = 6

integer :: i_hf_start = 4
...
real(kind=dp), parameter :: y0  =  0.0E0_dp
real(kind=dp), parameter :: y1  =  1.0E-1_dp

real(kind=dp), parameter :: z0  =  0.0E0_dp
real(kind=dp), parameter :: z1  =  1.0E0_dp
...
jmax = jmax * factor
kmax = kmax * factor
k_hf_start = (k_hf_start-1) * factor + 1
k_hf_end = k_hf_end * factor
i_hf_start = (i_hf_start-1) * factor + 1
i_hf_end = i_hf_end * factor
...
write(ioout)  BC_SYMMETRY,1,k_hf_start,1,block(b) % nCell(1)

write(ioout)  BC_SYMMETRY,k_hf_start+1,k_hf_end,1,i_hf_start

write(ioout)  BC_SYMMETRY,k_hf_start+1,k_hf_end,i_hf_end+1,block(b) % nCell(1)

write(ioout)  BC_SYMMETRY,k_hf_end+1,block(b) % nCell(3),1,block(b) % nCell(1)

write(ioout) BC_HEATFLUX,k_hf_start,k_hf_end,i_hf_start,i_hf_end
write(ioout) ((hf_wall,k = k_hf_start,k_hf_end),i  = i_hf_start,i_hf_end)


!!! NORD
write(ioout) 1,BC_SYMMETRY,1,block(b) % nCell(3),1,block(b) % nCell(1)
!!! BACK
write(ioout) 1,BC_ISOTHERMAL,1,block(b) % nCell(2),1,block(b) % nCell(1)
write(ioout) ((temp_wall,i = 1,block(b) % nCell(1)),j=1,block(b) % nCell(2))
!!! FRONT
